...
def backup_s3(s3_path: str):
    """Copy s3_path to s3_path.bak (blocking). Silently ignores missing files."""
    subprocess.run(['aws', 's3', 'cp', s3_path, f"{s3_path}.bak"], capture_output=True)
...
def save_ode_viz(
    ...
    out_path = os.path.join(run_dir, "plots", f"ode-step{step}.png")
