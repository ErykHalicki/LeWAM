...
class DiT(nn.Module):
    """
    Diffusion Transformer operating on pre-extracted VJEPA2 patch embeddings.

    x: (B, N, jepa_dim)  — noisy patch embeddings from VJEPA2
    t: (B,)              — diffusion timesteps
    y: (B,)              — class labels

    Returns (B, N, jepa_dim) denoised patch embeddings,
    or (B, N, jepa_dim * 2) when learn_sigma=True.
    """
    def __init__(
        self,
        num_patches,
        ...
        self.out_channels = jepa_dim * 2 if learn_sigma else jepa_dim

        self.x_embedder = nn.Linear(jepa_dim, hidden_size, bias=True)
    ...
    def forward(self, x, t, y):
        ...
        for block in self.blocks:
            x = block(x, c)
        x = self.final_layer(x, c)                 # (B, N, out_channels)
        return x
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        ...
        eps, rest = model_out[:, :, :self.jepa_dim], model_out[:, :, self.jepa_dim:]
...
def get_1d_sincos_pos_embed(embed_dim, num_patches):
    ...
    return emb  # (num_patches, embed_dim)
