...
sbatch --array=0-$((NUM - 1))%40 src/lewam/training/scripts/lerobot_conversion/convert_lerobot_v21_to_v30.sh
```
...
```bash
python src/lewam/training/scripts/lerobot_conversion/list_all_lerobot_dataset_dirs.py /cluster/scratch/ehalicki/lerobot_community_dataset
```

Then verify all remaining datasets load correctly with the official LeRobot loader:
```bash
python src/lewam/training/scripts/lerobot_conversion/verify_datasets.py /cluster/scratch/ehalicki/lerobot_community_dataset
```
