...
class VJEPA2VideoPreprocessor(nn.Module):
    ...
    Each camera is independently resized to crop_size x crop_size,
    ...
    def forward(self, frames):
        ...
        flat = rearrange(frames.float(), 'b n t c h w -> (b n t) c h w')
        resized = resize(flat, [self.crop_size, self.crop_size], antialias=True)
        resized = rearrange(resized, '(b n t) c h w -> b n t c h w', b=B, n=N, t=T)
        stitched = rearrange(resized, 'b n t c h w -> b c t h (n w)')
