...
if [ "$NO_VJEPA" = "0" ]; then
    if [ ! -f ".cache/vjepa2_1_vitb_dist_vitG_384.pt" ]; then
        echo "Downloading V-JEPA weights..."
        curl -L -o .cache/vjepa2_1_vitb_dist_vitG_384.pt \
...
# 4. Install
uv pip install -e .
./src/lewam/scripts/init/install_aws.sh
